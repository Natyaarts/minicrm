                    ...
                    <div className="fixed inset-0 bg-black/40 z-[100] flex items-center justify-center p-4">
                        ...
                        >
                            ...
                            <form onSubmit={handleSaveUser} className="px-6 py-5 space-y-4">
                                ...
                                {(userModalMode === 'create' ? newUser.role : editUser?.role) === 'SALES' && (
                                    <div className="grid grid-cols-1">
                                        <label className="block text-xs font-semibold text-slate-500 mb-1">Sales Section</label>
                                        <div className="relative">
                                            <select
                                                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:border-indigo-400 outline-none text-sm text-slate-700 transition-all appearance-none"
                                                value={userModalMode === 'create' ? (newUser.sales_section || 'BOTH') : (editUser?.sales_section || 'BOTH')}
                                                onChange={e => userModalMode === 'create' ? setNewUser({ ...newUser, sales_section: e.target.value }) : setEditUser({ ...editUser, sales_section: e.target.value })}
                                            >
