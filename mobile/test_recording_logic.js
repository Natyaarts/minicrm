...
async function runTests() {
  ...
  const callSitara = { id: 101, customer_number: '+919515207159', date: new Date('2026-09-21T10:00:00Z').toISOString() };
  const callNandini = { id: 102, customer_number: '+919881760496', date: new Date('2026-09-21T10:01:00Z').toISOString() };
  ...
  const callNazarin1 = { id: 201, customer_number: '+917593905516', date: new Date('2026-09-21T09:00:00Z').toISOString() };
  const callNazarin2 = { id: 202, customer_number: '+917593905516', date: new Date('2026-09-21T11:00:00Z').toISOString() };
  ...
  // Mark consumed on successful upload
  ...
  // Simulate app restart: re-fetch from AsyncStorage
  ...
  // Another call to Sitara later tries to find a recording
  const callSitara2 = { id: 103, customer_number: '+919515207159', date: new Date('2026-09-21T12:00:00Z').toISOString() };
  ...
  // Upload success marks it consumed
  await markRecordingConsumed('recording_5m08s.m4a');
  // File still in physicalFiles
  assert(physicalFiles.has('recording_5m08s.m4a'), 'Original recording remains PHYSICALLY present on device storage');

  // Test 5: Failed upload is still retryable
  console.log('\n[Suite 5: Failed Upload Retryability]');
  const unconsumedFile = 'failed_upload_rec_999.m4a';
  // If upload fails, markRecordingConsumed is NOT called
  ...
  const unknownCall = { id: 301, customer_number: '+919111111111', date: new Date('2026-09-21T10:00:00Z').toISOString() };
  ...
  // Test 7: Duration synchronization
  console.log('\n[Suite 7: Duration Synchronization]');
  const recordingDurationSeconds = 308; // 5:08
  let displayedCallDuration = 45; // initially 45s
